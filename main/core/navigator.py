...
class Navigator:
    ...
    GLOBAL_APPLICATION_NAME = 'Application'
    ...
    def get(cls, user_path: str):
        ...
        raise Exception('Не найден класс-наследник Application в данной директории\n'
                        ...
                        f'{cls.GLOBAL_HOME_PATH.replace(".", "/") + "/" + user_path}')
    ...
    def dir(cls, user_path: str):
        ...
        user_path = '{0}/{1}'.format(
            str(home_dir.__path__).replace('_NamespacePath([\'', '').replace('\'])', ''), user_path)
    ...
    def get_class(cls, user_path: str):
        user_path = cls.GLOBAL_HOME_PATH + '.' + user_path.replace('/', '.')
        ...
        for clsmember in clsmembers:
            if clsmember[0] == cls.GLOBAL_APPLICATION_NAME and \
                    str(clsmember[1]) == cls.GLOBAL_APPLICATION_PATH:
                safe_flags[0] += 1
                clsmembers.remove(clsmember)
                break
        if safe_flags[0] < 1:
            raise Exception('Не пройдён ПЕРВЫЙ флаг безопасности, проверьте правильность наследования класса {0}'.format(clsmembers))
        ...
        for clsmember in clsmembers:
            ...
            if len(childs) > depth and \
                ...
                depth = len(childs)
        if safe_flags[1] < 1:
            raise Exception('Не пройдён ВТОРОЙ флаг безопасности, проверьте правильность наследования класса {0}'.format(clsmembers))
